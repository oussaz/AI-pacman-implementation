    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos,prob in newPosDist.items():
                # Conditioning on oldPos : P(new position at t+1) = sum(P(new position at t+1 | old position at t)*P(old position at t))
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in self.beliefs:
            self.particles = self.particles + [p]*int(self.beliefs[p]*self.numParticles)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.beliefs:
                ...
                # P(X_t | e_t) = alpha * P(X_t)*P(e_t|X_t)
                distWeights[p] = emissionModel[trueDistance]*self.beliefs[p]    

            if distWeights.totalCount() == 0:
                self.initializeUniformly(gameState)

            else:
                distWeights.normalize()
                self.particles = [util.sample(distWeights) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState,oldPart))) for oldPart in self.particles]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        oneGhostPos = [p for p in self.legalPositions]
        ...
        for p in self.beliefs:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None :
                    ...
                    # B(x_t) <- B(x_t)*P(e_t|x_t = (x_ghost,y_ghost))
                    prob *= emissionModels[i][trueDistance]
            allPossible[p] = allPossible[p] + prob
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                replacingParticle.append(util.sample(newPosDist))
            newParticles.append(tuple(replacingParticle))
